// var title = document.queryselector("h1");
// title.innerHTML = "This is the title from code(1).js"

var button = document.querySelector("#cv");

button. addEventListener("click", myfunction);

function myfunction(){
    alert("Let me tell you more about myself!")
}

var mycode = document.createElement("div");
// change basic attributes 
//mynode.id = "work1_intro";
mynode.innerhtml ="The work is an exhibiton";
mynode.style.color = "blue";

// add event listerner
mynode.addEventListener("click",welcomeTowrk1 );
document.queryselector("#my_work1").appendchild(mynode);

function welcomework1() {
    mycode.innerHTML = "Thank you for your interest in my work!"
}
